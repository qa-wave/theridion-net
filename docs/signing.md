...
Generated locally via `pnpm dlx @tauri-apps/cli signer generate -w ~/.tauri/theridion-net.update.key`.

- Public key — embedded in `src-tauri/tauri.conf.json` `plugins.updater.pubkey`
- Private key — `~/.tauri/theridion-net.update.key` (gitignored)
...
7. Set GitHub secrets:
   ```bash
   gh secret set APPLE_CERTIFICATE --repo qa-wave/theridion-net < <(base64 -i developer-id.p12)
   gh secret set APPLE_CERTIFICATE_PASSWORD --repo qa-wave/theridion-net
   gh secret set APPLE_SIGNING_IDENTITY --repo qa-wave/theridion-net  # e.g. "Developer ID Application: Tomáš Mertin (TEAMID12)"
   ```
...
8. For notarization (required for distribution outside App Store):
   ...
   ```bash
   gh secret set APPLE_ID --repo qa-wave/theridion-net             # your Apple ID email
   gh secret set APPLE_PASSWORD --repo qa-wave/theridion-net       # the app-specific password
   gh secret set APPLE_TEAM_ID --repo qa-wave/theridion-net        # 10-char Team ID
   ```
...
4. Set GitHub secrets:
   ```bash
   base64 -i theridion-net.pfx | tr -d '\n' | gh secret set WINDOWS_CERTIFICATE_PFX_BASE64 --repo qa-wave/theridion-net
   gh secret set WINDOWS_CERTIFICATE_PASSWORD --repo qa-wave/theridion-net
   ```
...
Get-AuthenticodeSignature "C:\Program Files\Theridion Net\theridion-net.exe"
